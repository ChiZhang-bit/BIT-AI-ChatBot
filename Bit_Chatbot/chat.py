...
global sent_vec
global answer
sent_vec = pd.read_csv('./cache/sentence2vec.csv', encoding="utf-8", header=None, sep='|', error_bad_lines=True)
    ...
    try:
        ...
        if code != 0:
            ...
            if result == ',' or result=='.' or result=='。' or result=='？' or result=='!':
                ...
                global sent_vec
                global answer
                #t.insert(END, 'Me: '+ result +'\n')
                print("Me: %s" % (result))
                inputs_str = result
                # 需要把之前计算的概率值去掉，要不然计算相似度时候维度不同
                sent_vec = sent_vec.iloc[:, 0:202]
                response = chat_main(sent_vec, vec_model, mybot, T, inputs_str)
                answer = response
                #t.insert(END, 'ChatBot: ' + response + '\n')
...
def run():
    global wsParam
    wsParam = Ws_Param(APPID='60af0952', APIKey='438309e768a610da106394262f60e071',
                       APISecret='ZTA1NDMzMDQ4YmZjOTVjNDIzNzgxODcz')
...
def get_response(response_frame, inputs_str):
    ...
    if response_frame is None:
        ...
        # 这里用levenshtein ratio进行过滤，只选大于0.6的匹配度的问题
...
def chat_main(sent_vec, vec_model, mybot, T, txt):
